...
| **P2** | **`delphi` CLI surface** | No command to rebuild, inspect index stats, or verify recall. | e.g. `delphi memory rebuild`, `delphi memory stats` — not yet added to the existing `delphi` CLI (`common/cli.py`). |
