...
3. **`delphi` CLI surface.** `CLAUDE.md §6` envisions a single `delphi` entry point. The registry currently has no CLI (e.g. `delphi registry verify <stream>`, `delphi registry show <experiment>`). Add commands once `common/cli.py` exists.
